...
class UserViewTestCase(APITestCase):
    def setUp(self):
        self.forms = {
            "with_full_data": {"username": "test", "password": "test"},
            "with_full_data2": {"username": "test2", "password": "test2"},
            "without_username": {"password": "test"},
            "without_password": {"username": "test"},
            "wrong_password": {"username": "test", "password": "wrong"},
            "wrong_username": {"username": "wrong", "password": "test"},
            "empty_form": {},
        }
        # Create a user
        self.client.post("/api/user/", self.forms["with_full_data"], format="json")

    def test_signup(self):
        def response(form: str):
            return self.client.post("/api/user/", self.forms[form], format="json")
    ...
    def test_signin(self):
        def response(user_type: str):
            return self.client.post("/api/token/", self.forms[user_type], format="json")
...
class TaskViewTestCase(APITestCase):
    def setUp(self):
        # Create a user
        self.client.post(
            "/api/user/", {"username": "test", "password": "test"}, format="json"
        )
    ...
    def test_post(self):
        forms = {
            "with_full_data": {"title": "test3", "description": "test3"},
            "without_title": {"description": "test3"},
            "without_description": {"title": "test3"},
            "empty_form": {},
        }

        def response(form: str, auth: bool = True):
    ...
    def test_complete(self):
        ...
        def response(pk: int = 1, auth: bool = True):
